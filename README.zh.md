...
> [!NOTE]
> **预发布。** Auto Mode 已在真机上完整验证 —— 全新安装时，从按电源键到连上大约 80 秒。目前还没有
> 打标签的 release，请自行从源码构建。被封锁网络下的备用路径（CDN 镜像 → 发现的代理）能编译、有单元
> 测试，但还没有在真正封锁 GitHub 的网络里跑过。
...
```mermaid
flowchart LR
    A["直连<br/>raw.githubusercontent.com"] -->|被封| B["CDN 镜像<br/>jsdelivr · githack"]
...
目前还没有打标签的 release，所以唯一的路径是从源码构建。有了之后会以 APK 形式发在
