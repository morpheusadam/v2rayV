...
> [!NOTE]
> **Предрелиз.** Auto Mode проверен на реальном устройстве от начала до конца — от нажатия
> питания до подключения около 80 секунд на свежей установке. Тегированного релиза пока нет,
> собирайте из исходников. Обходной путь для заблокированных сетей (зеркала CDN → найденные
> прокси) компилируется и покрыт юнит-тестами, но ещё не проверялся в сети, где GitHub
...
```mermaid
flowchart LR
    A["напрямую<br/>raw.githubusercontent.com"] -->|закрыто| B["зеркала CDN<br/>jsdelivr · githack"]
...
Тегированного релиза пока нет, так что единственный путь сегодня — сборка из исходников.
Когда он появится, он будет на странице [Releases](../../releases) в виде APK.
